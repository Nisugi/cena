...
def fill(t, pr="her"):
    return (t.replace("{pronoun}", pr).replace("{Pronoun}", "Her").replace("{target}", "you")
             .replace("{direction}", "north"))

texts_prep = [fill(r[3],pr) for r in rows if len(r) > 3 and r[1] == "spell_prep" for pr in ("her","his","its","him")]
texts_all = [(r[0], r[1], fill(r[3],pr)) for r in rows if len(r) > 3 for pr in ("her","his","its","him")]
