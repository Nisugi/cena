...
| `plan/37-spell-behaviors.md` | **spellactive, ewaggle, spellcaster**: measured and staged; Stages 1-2 BUILT (what the spell table dropped, joined back: `cena-model/src/spells/extras.rs`; durations and costs evaluated for the character: `spells/expr.rs`, `state/spell_time.rs`), Stage 3 (the casting step, `cena-behavior/src/cast.rs`), Stage 4 (spellactive as `;keep`, `keep.rs`) |
