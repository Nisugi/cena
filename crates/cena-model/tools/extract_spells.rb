...
File.open(DEST, 'w') do |out|
  ...
  # WHICH COPY, and this is not bookkeeping. Two effect-list.xml files exist on
  # the author's machine and they differ by exactly the cooldown feature:
  #
  #   C:/Gemstone/lich-5/data       Sep 13  230,659 bytes  5 cooldowns
  #   E:/Gemstone/dev/lich-5/data   Sep 11  230,225 bytes  0 cooldowns
  #
  # The older one predates PR #1597. Cutting from it would produce a table
  # where `with_cooldowns()` is empty and the invariant test passes VACUOUSLY
  # over zero rows -- a green suite reporting a feature that is not there. The
  # right file was picked by luck the first time; these lines mean the next
  # person does not need luck, and the cooldown count below fails loudly.
  out.puts "# source\t#{File.expand_path(SOURCE)}"
  out.puts "# source-mtime\t#{File.mtime(SOURCE).strftime('%Y-%m-%d %H:%M:%S')}"
  out.puts "# spells\t#{rows.length}"
  out.puts "# cooldowns\t#{rows.count { |r| !r[13].empty? || !r[14].empty? }}"
