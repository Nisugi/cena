...
#[test]
fn every_attribute_of_an_item_survives_verbatim() {
    // Rule 2.2a: the typed fields are what today's consumer needs; `attrs`
    // is what stops tomorrow's from being unable to see an attribute the
    // wire adds.
    let menu = menus(REAL_MENU).pop().expect("a menu");
    let item = menu
        .items
        .iter()
        .find(|i| i.noun.as_deref() == Some("codex"))
        .expect("the codex item");
    assert_eq!(
        item.attrs,
        vec![
            ("coord".to_owned(), "2524,1906".to_owned()),
            ("noun".to_owned(), "codex".to_owned()),
        ]
    );
}
