...
const CAP_EXCEPTIONS: &[CapException] = &[
    ...
    // The three creature tables, for exactly the reason crit_tables.tsv states
    // below: they are DATA, one fact per line, with no seam to split on. The
    // fourth, creatures.tsv, is 628 lines and needs no exception -- which is
    // the ratchet working: only the files that genuinely exceed the cap carry
    // a justification, and the cap still binds the one that does not.
    //
    ...
    CapException {
        ...
        cap: 4200,
        justification: "3,862 death, flee, arrival and decay lines for 627 creatures, one per \
                        row, generated by tools/extract_creatures.rb from Lich's bestiary. Data \
                        ...
                        static mut past the other bans; the cap is 4200 rather than unbounded so \
                        a regeneration that added ~10% still goes red.",
    },
    CapException {
        path: "crates/cena-model/data/creature_attacks.tsv",
        cap: 1800,
        justification: "1,603 physical attacks across 627 creatures, one per row, with the \
                        attack strength as a scalar or a lo..hi range. Data with no functions in \
                        ...
                        templates: static data shipping as data files. MEASURED: 1,603 rows, \
                        six of which carry an unparsed as_raw value because the source is \
