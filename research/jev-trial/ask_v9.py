...
# The game's own terrain for each room, from the author's survey sessions
# (README section 11i). Codes -> names as the survey recorded them.
_META = json.load(open(Path(__file__).parent / "results" / "roommeta_from_survey.json"))
_TERRAIN = {0: "none set", 1: "hard, flat", 2: "plain dirt", 3: "sandy",
            4: "grassland", 5: "barren scrub", 6: "deciduous forest",
            7: "coniferous forest", 8: "tropical", 9: "riparian", 10: "rough",
            11: "hilly", 12: "mountainous", 13: "cultivated",
            14: "subterranean", 15: "muddy wetlands", 16: "icy glacier",
            17: "ruins", 18: "open water"}
...
CRITERIA = {
    ...
    "same floor": "The character stays on the SAME LEVEL. The two rooms are "
                  "side by side: both on the street, both at water level, both "
                  "on one floor of a building. This INCLUDES walking out onto "
                  "a bridge, footbridge, plank, pier, dock, walkway, catwalk "
                  "or causeway: a span is level with the ground at either end, "
                  "so crossing one does not change your floor even though the "
                  "ground may fall away beneath it. It also includes a path, "
                  "trail, road or alley that merely leads somewhere else on "
                  "the same level.",
}
...
GUIDANCE = (
    ...
    "up, or two ways down. Where a room has game_terrain, it is the game's "
    "own label. 'subterranean' means the room is underground: an exit from a "
    "room that is not subterranean into one that is goes down, and the "
    "reverse goes up. Two subterranean rooms can still be on different "
    "levels."
)
...
def state_for(item):
    ...
    def side(r):
        ...
        if g is not None and g.get("terrain") in _TERRAIN:
            out["game_terrain"] = _TERRAIN[g["terrain"]]
        if r["id"] in GROUND:
