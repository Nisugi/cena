...
use cena_behavior::travel::{MAX_REPLANS, Said, Trip, Why};
...
fn walk(
    ...
    for _ in 0..200 {
        match trip.tick(&map, walker, Some(here)) {
            Said::Send(command) => {
...
fn a_move_under_way_is_not_sent_twice() {
    ...
    assert_eq!(
        trip.tick(&map, &walker, Some(RoomId(1))),
        Said::Send("east".into())
    );
    // Still in 1, and ticked again -- by another frame, by the clock.
    assert_eq!(trip.tick(&map, &walker, Some(RoomId(1))), Said::Hold);
    assert_eq!(
        trip.tick(&map, &walker, None),
        Said::Hold,
        "the room is not known"
    );
    assert_eq!(
        trip.tick(&map, &walker, Some(RoomId(2))),
        Said::Send("east".into())
    );
}
...
fn a_trip_that_cannot_be_made_says_why_and_goes_on_saying_it() {
    ...
    assert_eq!(
        lost.tick(&map, &walker, Some(RoomId(1234))),
